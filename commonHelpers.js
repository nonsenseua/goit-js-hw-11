...
</li>`).join("")}function f(a){const r="https://pixabay.com/api/",s="45326372-cb2430b3eb7244718740f39d3",o=new URLSearchParams({key:s,q:a,image_type:"photo",orientation:"horizontal",safesearch:"false"});return fetch(`${r}?${o}`).then(e=>{if(!e.ok)throw new Error(e.status);return e.json()})}const n=document.querySelector(".gallery"),y=document.querySelector(".js-search-form"),l=document.querySelector(".loader");l.style.display="none";y.addEventListener("submit",d);function d(a){a.preventDefault();const r=a.currentTarget,s=r.elements.query.value.trim().toLowerCase();if(s===""){n.innerHTML="",c.error({message:"Please enter a search query."});return}l.style.display="block",f(s).then(o=>{const e=m(o);n.innerHTML=e,h.refresh()}).catch(g).finally(()=>{l.style.display="none",r.reset()})}function g(a){c.error({message:"Sorry, there are no images matching your search query. Please try again!"}),n.innerHTML="",l.style.display="none"}let h=new p(".gallery .gallery-link",{captionsData:"alt",captionDelay:250});c.settings({class:"izi-toast",position:"topRight",backgroundColor:"rgba(239, 64, 64, 1)",progressBarColor:"rgba(181, 27, 27, 1)",theme:"dark"});
